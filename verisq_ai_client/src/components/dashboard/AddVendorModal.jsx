...
function AddVendorModal({ isOpen, onClose, companyDomain, onRequireEmail }) {
  ...
  const validateDataClassification = (value) => {
    return value ? "" : "Please select data classification";
  };
  ...
  const validate = () => {
    ...
    const vendorNameRegex = /^[A-Za-z0-9\s&.-]+$/;
    ...
    if (!vendorName.trim()) {
      ...
      newErrors.vendorName = "Vendor Name cannot exceed 100 characters";
    } else if (!vendorNameRegex.test(vendorName.trim())) {
      newErrors.vendorName = "Vendor Name contains invalid characters";
    }
    ...
    const cleanedDomain = vendorDomain
      .trim()
      .replace(/^https?:\/\//, "")
      .replace(/^www\./, "")
      .toLowerCase();
    ...
    if (!cleanedDomain) {
      ...
      newErrors.vendorDomain = "Enter valid domain (example.com)";
    } else if (companyDomain && cleanedDomain === companyDomain.toLowerCase()) {
    ...
    if (sendQuestionnaire) {
      if (!vendorEmail.trim()) {
        ...
        if (!emailRegex.test(vendorEmail.trim())) {
          ...
          const emailDomain = vendorEmail.split("@")[1]?.toLowerCase();
    ...
    <div className="avm-overlay" onClick={handleOverlayClick}>
      <div className="avm-container">
        ...
        <div className="avm-body">
          <div className="avm-field">
            <label className="avm-label">
              Vendor Name <span className="avm-asterisk">*</span>
            </label>
            <input
              type="text"
              className="avm-input"
              ...
              onChange={(e) => {
                ...
                if (!value.trim()) {
                  ...
                  error = "Vendor Name cannot exceed 100 characters";
                } else if (!vendorNameRegex.test(value.trim())) {
          ...
          <div className="avm-field">
            <label className="avm-label">
              Vendor Domain <span className="avm-asterisk">*</span>
            </label>
            <input
              type="text"
              className="avm-input"
              ...
              onChange={(e) => {
                ...
                if (!cleanedDomain) {
                  error = "Vendor Domain is required";
                } else if (!domainRegex.test(cleanedDomain)) {
                  error = "Enter valid domain (example.com)";
                } else if (
                  companyDomain &&
                  cleanedDomain === companyDomain.toLowerCase()
                ) {
                  error =
                    "Vendor domain must be different from your company domain";
                }
          ...
          <div className="avm-field">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setErrors((prev) => ({
                  ...prev,
                  dataClassification: validateDataClassification(value),
                }));
              }}
            >
        ...
        <div className="avm-footer">
          ...
          <button className="avm-submit-btn" onClick={handleSubmit}>
            + Add Vendor
          </button>
        </div>
      </div>
