...
export default function VendorDetailsPage() {
  ...
  const fetchVendor = async () => {
    try {
      ...
      const response = await apiRequest(`/api/dashboard/vendor/${id}`);
  ...
  const questionnaire = selectedAssessment?.questionnaire;

  const scorecard = selectedAssessment?.scorecard;

  const findings = selectedAssessment?.findings || [];

  const responses = selectedAssessment?.responses || [];

  const handleAssessmentSelect = async (historyItem) => {
    try {
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      a.download = `${data.vendor.name}_report.pdf`;
    ...
    <>
      ...
      <main className="dashboard-main">
        <VendorHeader
          ...
          isSendingQuestionnaire={isSendingQuestionnaire}
          ...
          onCancelQuestionnaire={async () => {
            ...
            try {
              await cancelQuestionnaire(questionnaire.id);

              await fetchVendor();

            //   alert("Questionnaire cancelled successfully.");
            } catch (err) {
              console.error(err);

              alert(err.message || "Failed to cancel questionnaire.");
            }
          }}
        ...
        <div className="vd-details-layout">
          ...
          <div className="vd-details-layout__content">
            ...
            <QuestionnaireDetailsCard questionnaire={questionnaire} />
